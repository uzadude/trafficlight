...
# we need smooth transfer between the states that ensures yellow light
def getProgramTransfer(programs, lastProgramPointer, curProgramPointer):
    return ''.join(['y' if programs[lastProgramPointer][0][i] == 'G' and programs[curProgramPointer][0][i] == 'r' else
                    programs[lastProgramPointer][0][i] for i in range(len(programs[curProgramPointer][0]))])
...
class SmartTL(Smart):
    def getMinProgram(self):
        ...
        for j in range(len(self.programs)):
            ...
            print program, s, [self.lanes_cars_waiting_map[self.lanes_lst[i]] for i in range(len(program)) if
                               program[i] == 'G']
